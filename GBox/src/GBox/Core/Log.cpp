...
void Log::Init() {
    ...
    logSinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("GBox.log", true));
    ...
    //创建日志记录器
    s_CoreLogger = std::make_shared<spdlog::logger>("GBox", begin(logSinks), end(logSinks));			// Logger with range on sinks: logger(std::string name, It begin, It end) 传入所有需要使用的日志目标
